...
No AI — pure rule logic based on LATAM benchmarks from the DeepLook metrics framework.

LATAM benchmarks used:
- First response time benchmark: 5 minutes (78% of LATAM consumers buy from the first responder)
...
def generate_recommendations(
    ...
    # 1. First response time — the single most important LATAM metric
    if first_response_time_seconds is not None and first_response_time_seconds > 300:
        ...
        recs.append(
            f"Tu tiempo de primera respuesta es {human}. El 78% de los consumidores en LATAM "
            f"compran del primer negocio que les responde. Reducir este tiempo a menos de 5 minutos "
            f"puede aumentar significativamente tu tasa de conversión."
        )
    elif avg_response_time_seconds is not None and avg_response_time_seconds > 300 and first_response_time_seconds is None:
        # Fallback to avg if first RT is not available
        human = _fmt_seconds(avg_response_time_seconds)
        recs.append(
            f"Tu tiempo promedio de respuesta es {human}. El 78% de los consumidores en LATAM "
            f"compran del primer negocio que les responde. Considera configurar respuestas automáticas "
    ...
    if with_sentiment:
        ...
        if neg_pct > 20:
            ...
            topics_str = ", ".join(
                t for t, _ in Counter(neg_topics).most_common(3)
            )
            recs.append(
    ...
    # 5. Lost leads — with LATAM benchmark comparison
    ...
    if applicable:
        ...
        if lost_pct > 20:
            ...
            reasons_str = "; ".join(
                t for t, _ in Counter(lost_reasons).most_common(2)
            )
            recs.append(
                f"Estás perdiendo el {lost_pct:.0f}% de los leads. "
                f"El promedio de los negocios exitosos en LATAM con WhatsApp es del 35-42% de conversión. "
    ...
    if with_quality:
        ...
        if avg_quality < 6:
            ...
            dim_labels = {
                ...
                "speed_perception": "Velocidad de Percepción",
            }
    ...
    return recs[:5] if recs else [
        ...
        "y un servicio de calidad para mantener tu ventaja competitiva en LATAM."
    ]
